...
@network_bp.route("/network/dlist", methods=["POST"])
@jwt_set_user
def list_docker():
    """
    list all networks,
...
@network_bp.route("/network/ddetail")
@jwt_set_user
def network_detail_docker():
    """
    get detail of a network
    """
    network_name = request.values.get(consts.P_NETWORK_NAME)
    return docker_network_api.network_detail(network_name).to_json_str()


@network_bp.route("/network/dports", methods=["POST"])
@jwt_set_user
def list_interfaces_docker():
    """
    list all virtual interfaces,
    if user_name is given, list user's interfaces
    """
    user_name = request.values.get(consts.P_USER_NAME)
    return docker_network_api.list_interfaces(user_name).to_json_str()


@network_bp.route("/network/ddetailPort")
@jwt_set_user
def detail_interface_docker():
    """
    get a all virtual interfaces detail
    """
    name = request.values.get(consts.P_INTERFACE_NAME)
    return docker_network_api.interface_detail(name).to_json_str()


@network_bp.route("/network/dportAdd", methods=["POST"])
@jwt_set_user
def port_add_docker():
    """
    add virtual port to a internet
    ...
    ipaddress = request.values.get(consts.P_IP_ADDRESS)
    gateway = request.values.get(consts.P_GATEWAY)
    return docker_network_api.create_interface(name, network_name, ipaddress, gateway).to_json_str()


@network_bp.route("/network/dportDel", methods=["POST"])
@jwt_set_user
def port_del_docker():
    """
    delete virtual port from internet
    """
    name = request.values.get(consts.P_INTERFACE_NAME)
    return docker_network_api.delete_interface(name).to_json_str()


@network_bp.route("/network/dnetworkAdd", methods=["POST"])
@jwt_set_user
def network_add_docker():
    """
    add virtual network
    """
    network_name = request.values.get(consts.P_NETWORK_NAME)
    address = request.values.get(consts.P_NETWORK_ADDRESS)
    return docker_network_api.create_network(network_name, address).to_json_str()


@network_bp.route("/network/dnetworkDel", methods=["POST"])
@jwt_set_user
def network_del_docker():
    """
    delete virtual network
    """
    network_name = request.values.get(consts.P_NETWORK_NAME)
    return docker_network_api.delete_network(network_name).to_json_str()



@network_bp.route("/network/dcreateTop", methods=["POST"])
@jwt_set_user
def create_top_docker():
    """
    create top swarm network
    """
    network_addr = request.values.get(consts.P_NETWORK_ADDRESS)
    return docker_network_api.create_top_network(network_addr).to_json_str()


@network_bp.route("/network/ddeleteTop", methods=["POST"])
@jwt_set_user
def delete_top_docker():
    """
    delete top swarm network
    """
    return docker_network_api.delete_top_network().to_json_str()
