...
def create_container():
    ...
    res = service.create_container(image=image, name=container_name,
                             cpu_count=cpu_num, memory_limit=mem_limit,
...
def delete_container():
    container_id = request.values.get(consts.P_DOMAIN_UUID)
...
def update_container():
    ...
    res = service.update_container(container_id, mem_limit=mem_limit, new_name=new_name)
    return res.to_json_str()
...
def joinSwarm():
    ...
    res = service.join_swarm(remote_addrs,swarm_worker_token, advertise_addr, listen_addr)
    return res.to_json_str()
