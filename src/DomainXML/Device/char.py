from src.DomainXML.Device.device import Device, DeviceSeclabel
...
class CharSource(XMLBuilder):
    ...
    seclabels = XMLChildBuilder(DeviceSeclabel)
    ...
    bind_service = XMLProperty(
        "./../source[@mode='bind']/@service", is_int=True)
...
class _DeviceChar(Device):
    ...
    source = XMLChildBuilder(CharSource, is_single=True)

    target_address = XMLProperty("./target/@address")
    target_port = XMLProperty("./target/@port")
